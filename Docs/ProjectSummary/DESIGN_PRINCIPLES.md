...
- **基类设计**：KuKuData作为基础类，MythicalKuKuData继承自KuKuData，UnitData和HeroData作为独立的单位类型
...
## 4. 继承与多态原则
- **继承体系**：MythicalKuKuData继承KuKuData，体现类型层次
- **多态实现**：通过重写方法实现不同类型对象的差异化行为
- **接口统一**：提供统一的接口供其他系统调用
- **类型区分**：UnitData、HeroData、KuKuData、MythicalKuKuData有不同的属性和行为
...
## 11. 游戏机制设计原则
- **捕捉机制**：前5分钟捕捉野怪，野怪死亡后掉落灵魂、金钱、KuKu蛋
- **进化机制**：KuKu通过吸收野怪灵魂进化，提升稀有度和属性
- **融合机制**：进化到最终等级的KuKu可与特定机器人融合
- **装备机制**：融合后的终极单位可装备六件装备
- **防守机制**：使用捕捉到的KuKu和建造的防御设施抵御敌人
