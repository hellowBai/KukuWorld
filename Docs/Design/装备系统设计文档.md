# 宠物收集塔防游戏《Pet Collector》装备系统设计文档

## 1. 系统概述

装备系统是《Pet Collector》游戏的核心玩法之一，允许玩家为融合后的终极单位装备多达六件装备，以大幅提升战斗力。系统包含装备合成、装备穿戴、属性加成等功能。
...
public static EquipResult EquipWeapon(object unit, WeaponData weapon)
{
    ...
    else if (unit is MythicalPetData)
    {
        // 如果是宠物，暂时返回错误（宠物不能直接装备武器）
        return new EquipResult(false, "宠物无法直接装备武器，需要先与机器人融合！");
    }
    else
    {
...
public static List<WeaponData.WeaponType> GetCompatibleWeaponTypes(object unit)
{
    ...
    switch (unitData.Type)
    {
        ...
        case UnitData.UnitType.PetHybrid:
            // 宠物混合单位可能有特殊的兼容性
            compatibleTypes.AddRange((WeaponData.WeaponType[])System.Enum.GetValues(typeof(WeaponData.WeaponType)));
            // 移除某些不兼容的类型（示例）
            compatibleTypes.RemoveAll(t => t == WeaponData.WeaponType.Processor); // 处理器不适合宠物
            break;
    }
