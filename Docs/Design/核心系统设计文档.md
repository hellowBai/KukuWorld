# KuKu收集塔防游戏《Kuku Collector》核心系统设计文档

## 1. 系统概述

本文档详细描述了《Kuku Collector》游戏的核心系统设计，特别针对"前5分钟捕捉→后续防守"的独特玩法进行设计。其中收集系统为游戏外的图鉴系统，捕捉系统、战斗系统等为游戏内核心玩法系统。

## 2. 神秘KuKu数据系统

### 2.1 KukuData (基础KuKu数据结构)

#### 2.1.1 属性定义
```csharp
// 基础信息
public int Id { get; set; }                           // KuKu唯一标识
public string Name { get; set; }                      // KuKu名称
public string Description { get; set; }               // KuKu描述
...
### 2.2 MythicalKukuData (神话KuKu数据结构)
...
### 3.1 KukuCollectionManager (收集管理器)
...
#### 3.1.2 属性定义
```csharp
public static KukuCollectionManager Instance { get; private set; } // 单例实例

// 数据存储
private Dictionary<int, KukuData> allKukuDatabase;                   // KuKu数据库
private int currentPageIndex;                                      // 当前页面索引
private const int KUKUS_PER_PAGE = 12;                             // 每页KuKu数量
...
// 生成神话KuKu名字
private string GenerateMythicalKukuName(int id)
...
```csharp
// 生成野外KuKu
public static void GenerateWildKuku(Vector3 position, MythicalKukuData.MythicalRarity rarity)

// 攻击野生KuKu
public static bool AttackWildKuku(int kukuId, float damage, out string message)

// 尝试捕捉野生KuKu
public static CaptureResult AttemptCapture(int kukuId, MythicalKukuData capturerKuku)
...
// 检查位置是否有野生KuKu
public static bool HasWildKukuAtPosition(Vector3 position, float radius)
...
public int maxWildKukus = 10;                              // 最大野外KuKu数量
public float wildKukuRespawnTime = 30f;                    // 重生时间
...
// 事件
public event Action<Vector3> OnWildKukuSpotted;            // 发现KuKu事件
...
// 根据玩家等级获取随机稀有度
private MythicalKukuData.MythicalRarity GetRandomRarityForPlayerLevel()

// 玩家探索指定区域
public void ExploreArea(Vector3 position)

// 获取探索信息
public (Bounds bounds, int currentKukus, int maxKukus) GetExplorationInfo()
...
public GameObject wildKukuCardPrefab;                      // KuKu卡片预制体

// 信息显示
public TextMeshProUGUI selectedKukuNameText;               // KuKu名称
public TextMeshProUGUI selectedKukuRarityText;             // KuKu稀有度
public TextMeshProUGUI selectedKukuHpText;                 // KuKu生命值
public Slider selectedKukuHpSlider;                        // 生命值滑动条
...
// 野外KuKu被选中
private void OnWildKukuSelected(CaptureSystem.WildKuku wildKuku)
...
// 灵魂吸收操作
private void AbsorbSoulFromSelectedKuku()
...
private List<GameObject> activePets;                     // 活跃KuKu（玩家防守单位）
...
public bool CanCollectSoulsInCurrentPhase(BattleSystem.BattleState currentPhase)
...
public enum UnitType { Robot, Tank, Hybrid, KukuHybrid }  // KukuHybrid是KuKu与机器人融合后的单位
...
```csharp
// 制造类型
public ManufacturingType ManufactureType { get; set; }    // 制造类型
...
// 制造材料
public List<CraftingMaterial> RequiredMaterials { get; set; } // 制造所需材料
public struct CraftingMaterial
{
    ...
    public enum MaterialType { 
        Metal, Crystal, EnergyCore, SoulFragment, 
        TechComponent, RareEarth, DivineEssence 
    }
}
```
...
// 检查是否可以合成
public bool CanCombineWith(WeaponData other)

// 应用武器属性到单位
public void ApplyToUnit(UnitData unit)

// 移除武器属性
public void RemoveFromUnit(UnitData unit)

// 获取合成结果预览
public WeaponData GetCombineResult(WeaponData other)

// 检查是否适合单位
public bool IsCompatibleWith(UnitData unit)
```
...
public float SoulShards { get; set; } = 0f;             // 灵魂碎片（用于合成灵魂）
public float DivineSouls { get; set; } = 0f;            // 神圣灵魂（高级灵魂类型）

// KuKu收集（游戏内实际拥有的KuKu）
public Dictionary<int, KuKuData> CollectedPets { get; set; } // 收集的KuKu
public List<int> ActivePetTeam { get; set; }            // 当前KuKu队伍
...
public void AddSoulShards(float amount)
public void AddDivineSouls(float amount)

// 合成灵魂（将灵魂碎片合成完整灵魂）
public bool SynthesizeSouls(int shardCount)

// 合成神圣灵魂（将普通灵魂合成神圣灵魂）
public bool SynthesizeDivineSouls(int soulCount)

// 消耗灵魂
public bool ConsumeSouls(float amount)
public bool ConsumeSoulShards(float amount)
public bool ConsumeDivineSouls(float amount)

// 添加经验值
public bool AddExperience(float exp)

// 添加KuKu到收藏（游戏内）
public void AddPet(KuKuData pet)

// 通知收集系统解锁KuKu（游戏外）
public void NotifyPetUnlocked(int petId)

// 获取灵魂类型（根据当前拥有的灵魂比例决定）
public SoulType GetAvailableSoulType()

// 获取灵魂合成配方
public Dictionary<SoulType, (SoulType input, int count)> GetSoulSynthesisRecipes()
