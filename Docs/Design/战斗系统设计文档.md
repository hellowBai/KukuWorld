# 宠物收集塔防游戏《Pet Collector》战斗系统设计文档

## 1. 系统概述

战斗系统是《Pet Collector》游戏的核心玩法之一，采用"前5分钟捕捉→后续防守"的独特机制。玩家在前5分钟内捕捉宠物，之后使用捕捉到的宠物和建造的防御设施抵御不断来袭的敌人，守护重要据点。系统包含捕捉阶段、防守阶段、敌人AI、宠物战斗AI等子系统。
...
- **类型**: 游戏系统
- **依赖**: PlayerData, PetData, EnemyController, PetCombatController

### 2.2 NuwaDefenseSystem (守护系统)
- **职责**: 管理守护据点的防御战斗
- **类型**: 游戏系统
- **依赖**: BattleSystem, PlayerData, MythicalPetData
...
### 2.4 PetCombatController (宠物战斗控制器)
- **职责**: 管理宠物战斗AI
- **类型**: 实体控制器
- **依赖**: PetData, EnemyController
...
private List<GameObject> activePets = new List<GameObject>();    // 活跃宠物列表
...
private void SpawnSingleEnemy(int waveNum)
{
    try
    {
        ...
        enemyComponent.Initialize(health, speed, damage, new Transform[] { targetPoint }, EnemyType.WildPet);
...
{
    petData = data;
    attackCooldown = 2f / petData.Speed; // 攻击间隔与速度相关
}
```
...
### 7.1 数据流
```
GameManager → PlayerData → BattleSystem → PetCombatController
...
### 7.3 依赖关系
- BattleSystem 依赖 PlayerData 获取玩家宠物
- EnemyController 依赖战斗系统处理结果
- PetCombatController 依赖 PetData 获取属性