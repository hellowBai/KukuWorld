# 宠物收集塔防游戏《Pet Collector》宠物收集系统设计文档

## 1. 系统概述

宠物收集系统是《Pet Collector》游戏的核心组成部分，包含游戏内外两个层面的收集机制。游戏外的图鉴系统记录玩家已解锁的宠物，游戏内的收集系统管理玩家实际拥有的宠物。该系统与捕捉、进化、融合等系统紧密关联。

## 2. 系统架构

### 2.1 PetCollectionManager (宠物收集管理器)
- **职责**: 管理宠物图鉴数据库，处理图鉴收集进度
- **类型**: 单例系统类
- **依赖**: PetData, PlayerData, GameManager

## 3. PetCollectionManager (宠物收集管理器)
...
public struct CollectionPageData
{
    ...
    public List<PetCollectionEntry> pets;         // 该页面的宠物
    public int unlockedCount;                       // 已解锁数量
    public int totalCount;                          // 总数量
    public float completionRate;                    // 完成率

    public CollectionPageData(int index, List<PetCollectionEntry> p)
    {
        pageIndex = index;
        pets = p;
        unlockedCount = 0;
...
public List<PetCollectionEntry> GetPagePets(int pageIndex)
{
    ...
    int endIndex = System.Math.Min(startIndex + petsPerPage, allPets.Count);
...
public Dictionary<PetData.RarityType, (int total, int unlocked)> GetRarityStatistics()
{
    ...
    // 初始化所有稀有度的统计
    foreach (PetData.RarityType rarity in System.Enum.GetValues(typeof(PetData.RarityType)))
    {
        stats[rarity] = (0, 0);
    }
...
private void SetMythicalPetAttributes(MythicalPetData pet, MythicalPetData.MythicalRarity rarity)
{
    ...
    // 设置技能
    MythicalPetData.MythicalSkillType[] skills = System.Enum.GetValues(typeof(MythicalPetData.MythicalSkillType)) as MythicalPetData.MythicalSkillType[];
...
private void TriggerCollectionEvents()
{
    ...
    int totalPets = allPetDatabase.Count;
    int unlockedPets = (int)(overallProgress * totalPets);
    
    OnCollectionProgressChanged?.Invoke(unlockedPets, totalPets);
...
### 4.1 数据流
```
CaptureSystem → PetCollectionManager → PetCollectionEntry
PlayerData → PetCollectionManager → Sync Operation
UI → PetCollectionManager → Display Data
GameManager → PetCollectionManager → State Updates
```

### 4.2 事件流
- 捕捉成功 → PetCollectionManager 解锁宠物 → 触发解锁事件 → UI更新显示
- 玩家查看图鉴 → PetCollectionManager 提供数据 → UI展示图鉴
- 收集进度变化 → PetCollectionManager 计算进度 → 触发进度事件

### 4.3 依赖关系
- PetCollectionManager 依赖 GameManager 获取玩家数据
- CaptureSystem 依赖 PetCollectionManager 记录解锁状态
- UI系统依赖 PetCollectionManager 获取图鉴数据
- PlayerData 与 PetCollectionManager 同步收集状态