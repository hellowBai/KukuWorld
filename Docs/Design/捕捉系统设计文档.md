# 宠物收集塔防游戏《Pet Collector》捕捉系统设计文档

## 1. 系统概述

捕捉系统是《Pet Collector》游戏的核心玩法之一，专为"前5分钟捕捉→后续防守"的独特机制设计。玩家在前5分钟内捕捉野生神宠，每只野怪死亡后会掉落灵魂、金钱和宠物蛋。初级宠物拥有吸收核心灵魂的技能，有概率让宠物进化。系统包含野外神宠生成、限时捕捉机制、灵魂吸收、掉落物生成等子系统。
...
- **类型**: 静态系统类
- **依赖**: PlayerData, MythicalPetData

### 2.2 ExplorationSystem (探索系统)
- **职责**: 管理地图探索和神宠生成，控制5分钟捕捉期的刷新机制
- **类型**: 单例系统
...
- 管理捕捉过程中的削弱机制
- 处理灵魂吸收进化机制
- 生成野怪死亡后的掉落物（灵魂、金钱、宠物蛋）
- 提供捕捉相关的查询接口

### 3.3 数据结构

#### 3.3.1 WildPet (野外神宠结构)
```csharp
public struct WildPet
{
    public MythicalPetData petData;    // 神宠数据
...
public struct CaptureResult
{
    ...
    public MythicalPetData capturedPet; // 捕获的神宠数据
...
/// 生成野外神宠
/// </summary>
public static void GenerateWildPet(Vector3 position, MythicalPetData.MythicalRarity rarity = MythicalPetData.MythicalRarity.Celestial)
{
    try
    {
        // 创建野生神宠数据
        MythicalPetData wildPet = new MythicalPetData();
        wildPet.Id = UnityEngine.Random.Range(1000, 9999); // 随机ID
...
public static bool AttackWildPet(int petId, float damage, out string message)
{
    ...
    // 这里应该维护一个野生神宠列表，实际实现中需要管理这个列表
    // 暂时返回模拟结果
    
    message = "野生神宠受到攻击，生命值降低";
...
/// 尝试捕捉野生神宠
/// </summary>
public static CaptureResult AttemptCapture(int petId, MythicalPetData capturerPet = null)
{
    // 这里应该从野生神宠列表中找到指定的宠物
    // 暂时返回模拟结果
    
    MythicalPetData capturedPet = new MythicalPetData();
    capturedPet.Id = petId;
    capturedPet.Name = "被捕捉的神宠";
    capturedPet.Rarity = MythicalPetData.MythicalRarity.Celestial;
...
/// 生成野生神宠名称
/// </summary>
private static string GenerateWildPetName(MythicalPetData.MythicalRarity rarity)
{
...
/// 获取捕捉成功率
/// </summary>
public static float GetCaptureSuccessRate(int petId, MythicalPetData capturerPet = null)
{
    // 实际实现中需要根据宠物稀有度、生命值比例、捕捉者宠物等因素计算
    return 0.7f; // 模拟成功率
}
...
/// 检查指定位置是否有野生神宠
/// </summary>
public static bool HasWildPetAtPosition(Vector3 position, float radius = 1.0f)
{
    // 实际实现中需要检查野生神宠列表
    return false;
}
...
private void CheckAndRespawnWildPets()
{
    ...
    // 检查该位置是否已经有神宠
    if (!CaptureSystem.HasWildPetAtPosition(randomPos, 2f))
    {
        CaptureSystem.GenerateWildPet(randomPos, rarity);
...
public GameObject wildPetCardPrefab;                // 野外神宠卡片预制体

[Header("捕捉信息显示")]
public TextMeshProUGUI selectedPetNameText;         // 选中宠物名称
public TextMeshProUGUI selectedPetRarityText;       // 选中宠物稀有度
public TextMeshProUGUI selectedPetHpText;           // 选中宠物生命值
public Slider selectedPetHpSlider;                  // 选中宠物生命值滑动条
...
public Button usePetAttackButton;                   // 使用宠物攻击按钮
...
#### 5.3.2 状态数据
```csharp
private CaptureSystem.WildPet? selectedWildPet;     // 选中的野外神宠
private MythicalPetData selectedCapturerPet;        // 选中的捕捉者宠物
...
private void InitializeUI()
{
    ...
    if (attackButton != null)
        attackButton.onClick.AddListener(() => AttackSelectedWildPet(false));
    
    if (usePetAttackButton != null)
        usePetAttackButton.onClick.AddListener(() => AttackSelectedWildPet(true));
    
    if (captureButton != null)
        captureButton.onClick.AddListener(() => AttemptCaptureSelectedPet(false));
    
    if (useSpecialCaptureButton != null)
        useSpecialCaptureButton.onClick.AddListener(() => AttemptCaptureSelectedPet(true));
}
```
...
public void CloseCapturePanel()
{
    ...
    selectedWildPet = null;
    selectedCapturerPet = null;
}
```
...
{
    if (wildPetContainer == null || wildPetCardPrefab == null) return;
    
    // 清除现有卡片
    ClearWildPetCards();
    
    // 这里应该从CaptureSystem获取野外神宠列表
    // 暂时创建模拟数据
...
/// 清除野外神宠卡片
/// </summary>
private void ClearWildPetCards()
{
    foreach (Transform child in wildPetContainer)
    {
...
### 6.1 数据流
```
ExplorationSystem → CaptureSystem → WildPet data
...
### 6.2 事件流
- ExplorationSystem 触发 OnWildPetSpotted → UI 显示新神宠
- CaptureUI 触发捕捉操作 → CaptureSystem 处理结果
- CaptureSystem 捕捉成功 → PlayerData 添加宠物和资源
- ExplorationSystem 时间结束 → GameManager 切换到防守阶段

### 6.3 依赖关系
- CaptureUI 依赖 CaptureSystem 获取数据
- ExplorationSystem 依赖 CaptureSystem 管理神宠
