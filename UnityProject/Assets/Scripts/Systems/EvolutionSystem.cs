...
public class EvolutionSystem : MonoBehaviour
{
    ...
    /// 批量进化多个KuKu
    /// </summary>
    /// <param name="kukuIds">要进化的KuKu ID列表</param>
    /// <returns>进化结果列表</returns>
    public List<EvolutionResult> BulkEvolve(List<int> kukuIds)
    {
        List<EvolutionResult> results = new List<EvolutionResult>();
        
        foreach (int kukuId in kukuIds)
        {
            results.Add(AttemptEvolution(kukuId));
        }
    ...
    /// 获取玩家所有可以进化的KuKu
    /// </returns>
    public List<MythicalKukuData> GetEvolvableKukus()
    {
        if (playerData == null) return new List<MythicalKukuData>();
        
        List<MythicalKukuData> evolvableKukus = new List<MythicalKukuData>();
        
        foreach (var kuku in playerData.CollectedKukus)
        {
            if (kuku.Level >= 10 && playerData.Souls >= CalculateSoulRequirement(kuku))
            {
                evolvableKukus.Add(kuku);
            }
        }
        
        return evolvableKukus;
    }
    
    /// <summary>
    /// 设置进化参数
    /// </summary>
    public void SetEvolutionParameters(float baseSoulReq, float rarityMult, float levelMult)
    {
        baseSoulRequirement = baseSoulReq;
        rarityMultiplier = rarityMult;
        levelMultiplier = levelMult;
        
        Debug.Log($"进化参数已更新: 基础灵魂需求={baseSoulRequirement}, 稀有度倍数={rarityMultiplier}, 等级倍数={levelMultiplier}");
    }
